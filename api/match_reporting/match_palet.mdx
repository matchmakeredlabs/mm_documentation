...
- If useVectorSearch is specified, ses Vector Space Model (VSM) for text similarity matching
- Otherwise, uses strict keyword matching to generate results
...
|-----------|----------|-------------|
| `useVectorSearch` | No | If specifed, uses Vector Space Model as opposed to strict keyword matching |

With this parameter, the endpoint would be:
```
POST https://matchmakeredlabs.net/api/match/palet?useVectorSearch
```
